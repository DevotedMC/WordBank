...
/**
 * Tests the distribution of the results of hashing functions against TwoTailDistribution function
 * 
 * @author ProgrammerDan
 *
 */
public class TestTwoTailDistributionFunction extends TestFunctionBase {
	
	protected static final TwoTailDistribution function = new TwoTailDistribution(wb);
	 ...
	 */
	@Test
	public void ensureMD5TwoTailDistribution() {
		ensureTwoTailDistribution("MD5");
	}
	
	@Test
	public void ensureSHA1TwoTailDistribution() {
		ensureTwoTailDistribution("SHA-1");
	}
	
	@Test
	public void ensureSHA256TwoTailDistribution() {
		ensureTwoTailDistribution("SHA-256");
	}

	@Test
	public void ensureSHA512TwoTailDistribution() {
		ensureTwoTailDistribution("SHA-512");
	}
	
	public void ensureTwoTailDistribution(String hash) {
		...
		double maxAverageVariance = 350f;
		...
		// Measure additive variance against uniform
		int peakVariance = 0;
		double varianceAvg = 0d;
		for (int i = 0; i < multiplier; i++ ) {
			int localVariance = Math.abs((2 * uniformBucketSize) - (normals[i] + buckets[i]));
			if (localVariance > peakVariance) peakVariance = localVariance;
			varianceAvg += localVariance;
			// display woo
			//System.out.println(String.format("%3d: h%4d g%4d v%5d", i, buckets[i], normals[i], localVariance));
		}
